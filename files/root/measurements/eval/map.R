...
draw_map <- function(data) {
	...
	done <- myps2(paste0("map_", campaign), width=wi, height=he*1.5)
	...
	pcol <- "black"
	if(map_color_rsseye == T) {
		pval <- aggregate(data$frame_info.rcv_power, by=list(data$expno), FUN=median)$x
		pcol <- rssi_col[mySignal_mapping(pval)]
	}

	dbvals <- ""
	if(map_dbvals_rsseye == T) {
		dbvals <- paste0("\n", round(aggregate(data$frame_info.rcv_power, by=list(data$expno), FUN=median)$x), " dBm")
	}

	points(aggregate(data$rx_X, by=list(data$expno), FUN=median)$x, aggregate(data$rx_Y, by=list(data$expno), FUN=median)$x, pch=4, cex=0.4, col=pcol, lwd=2)

	text(aggregate(data$rx_X, by=list(data$expno), FUN=median)$x+(axis.x.max-axis.x.zero)*map_text_exp_x, aggregate(data$rx_Y, by=list(data$expno), FUN=median)$x+(axis.y.max-axis.y.zero)*map_text_exp_y, paste0(round(aggregate(data$real_dist, by=list(data$expno), FUN=median)$x), " m", dbvals), cex=0.3, col="blue")

	legend("topleft", paste0("Exp.: ", campaign), bty="n", cex=0.8)
	legend("bottomright", c("sender", "receiver"), pch=c(10,4), pt.cex=c(1, 0.4), lwd=c(1,2), lty=c(NA, NA))
