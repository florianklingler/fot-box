...
load_data_realdist <- function (campaign, expno, dist) {
	...
	pdr <- nrow(rx) / mean(data$payload.totalno)
	data$pdr <- pdr
...
done <- myps2(paste0("pdr_rsseye_", campaign), width=wi, height=he*1.5)
par(mar=c(3.8, 3.8,1.0,1.0))

plot.new()
plot.window(xlim=c(-2,maxdist), ylim=c(0, 1.1), xaxs='i', yaxs='i')
...
for(e in 1:nrow(exp)) {
	...
	if(nrow(res) > 0) {
		#b <- boxplot(at=mean(res$mydist), res$pdr, add=T, axes=FALSE, pars = list(boxwex = 1.8, staplewex = 0.5, outwex = 0.5, outpch=4, outcex=0.3))

		v.x = c(v.x, mean(res$mydist))
		v.y = c(v.y, mean(res$pdr))

		#text( mean(res$dist), b$stats[nrow(b$stats) , ]+0.5 , paste("n = ",mean(res$real_dist),sep=""), cex=0.3)
	}
}

lines(v.x, v.y, lwd=2, col="black")
points(v.x, v.y, cex=0.4, pch=16)
...
title(ylab="packet delivery rate", line=2.8)
box()
