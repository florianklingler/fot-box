...
load_data_realdist <- function (campaign, expno, dist) {
	...
	### GPSLogger ###
	rx_gpslogger_filename <- paste0("results/host_rx/", campaign, "/", expno, "/sockperf_",sockperf_type,"/gpslogger_rx_expno-",expno,"_dist-",dist,".csv")
	rx_gpslogger <- read.delim(rx_gpslogger_filename ,sep=",")

	tx_gpslogger_filename <- paste0("results/host_tx/", campaign, "/", expno, "/sockperf_",sockperf_type,"/gpslogger_tx_expno-",expno,"_dist-",dist,".csv")
	...
	### SOCKPERF ###
	tx_sockperf_filename <- paste0("results/host_tx/", campaign, "/", expno, "/sockperf_",sockperf_type,"/sockperf_tx_expno-",expno,"_dist-",dist,".csv")
	tx_sockperf <- read.delim(tx_sockperf_filename ,sep=",", skip=20, stringsAsFactors = FALSE) #skip first 20 lines as they include garbage, do not make factors

	if(nrow(tx_sockperf) < 1) {
		error("too little observations for sockperf")
	}

	data_sockperf <- tx_sockperf[1:(nrow(tx_sockperf) -1),] # drop last line as it includes garbage
	data_sockperf$campaign <- campaign
	data_sockperf$expno <- expno
	data_sockperf$dist <- dist
	data_sockperf$real_dist <- mean(data_gps$real_dist)

	data_sockperf$mydist <- data_sockperf$real_dist

	data_sockperf$latency <- as.numeric(data_sockperf$rxTime) - as.numeric(data_sockperf$txTime) # is wrongly treaten as character
	data_sockperf$latency <- data_sockperf$latency / 2 # calculate one-way latency
	data_sockperf$latency <- data_sockperf$latency * 1000 # convert to ms
	### /SOCKPERF ###

	return(data_sockperf)
}
...
load_data <- function (campaign, expno, dist) {
	...
	data_sockperf$mydist <- data_sockperf$dist
...
done <- myps2(paste0("sockperf_", sockperf_type, "_", campaign), width=wi, height=he*1.5)
par(mar=c(3.8, 3.8,1.0,1.0))

plot.new()
plot.window(xlim=c(-2,maxdist), ylim=c(0, max_delay), xaxs='i', yaxs='i')
...
for(e in 1:nrow(exp)) {
	...
	if(nrow(res) > 0) {
		b <- boxplot(at=mean(res$mydist), res$latency, add=T, axes=FALSE, pars = list(boxwex = 35, staplewex = 0.5, outwex = 1.0, outpch=4, outcex=0.3, outcol=rgb(0,0,0, .1)), plot=draw_boxplots)
...
legend("topleft", paste0("Exp.: ", campaign, "\n", toupper(sockperf_type)), bty="n", cex=0.8)

axis(1)
axis(2, las=2)
title(xlab="distance in m", line=2.8)
title(ylab="one-way delay in ms", line=2.8)
box()
